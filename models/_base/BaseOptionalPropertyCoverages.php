...
class BaseOptionalPropertyCoverages extends \yii\db\ActiveRecord
{
	...
	public function rules()
	{
		return [
			...
			[['accounts_receivable', 'additional_expense',
				'businessowners_burglary_robbery', 'computer_coverage',
				'customers_goods', 'agreement_one', 'agreement_two',
				'agreement_three', 'money_securities',
				'direct_damages', 'time_element',
				'building_glass', 'refrigerated_food',
				'refrigerated_property', 'add_mos',
				'burglary_of_money', 'theft_of_money',
				'tenant_Improvements_one', 'tenant_Improvements_a',
				'valuable_papers', 'ornamental_work'],
				'number'],
			...
			[['sf_32_value'], 'validateHoodAndDuctProtection'],
		];
	}
	...
	public function validateHoodAndDuctProtection($attr, $params) {
		if(!empty($_POST['Quotes']['location'])){
			$location = Occupancy::findOne($_POST['Quotes']['location']);
			if($location){
				if(!$this->$attr&&in_array($location->name,[
						'Restaurant', 'Bagel Shop - with cooking',
						'Bakeries - with cooking and selling',
						'Candy, Nut and Confectionery Store-Cooking',
						'Confectionery, Candy and Nut Store-Cooking',
						'Nut, Candy and Confectionery Store with Cooking',
						'Pizza Shop - with cooking',
						'Mini Mart with Cooking',
						'Bar'
					])){
					$this->addError($attr, 'Restaurant Hood and Duct Protection must be chosen.');
				}
			}
		}
	}
